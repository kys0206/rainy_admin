...
export default function SlideListPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (isConfirmed) {
      try {
        const response = await del(`/slide/delete/${id}`)
        const data = await response.json()

        if (data.ok) {
          alert('게시물이 성공적으로 삭제되었습니다.')
          setSlides(prevTrips => prevTrips.filter(slide => slide._id !== id))
        } else {
  ...
  return (
    <div>
      <div className="bg-lightblue">
        <div className="p-3">
          <div>
            <p className="text-xl font-bold">슬라이드 목록</p>
          </div>
        </div>
      </div>
                ...
                <tbody>
                  {currentItems.map((slide, index) => (
                    <tr
                      ...
                      <td className="px-6 py-4">{slide.content}</td>
                      <td className="px-6 py-4">{slide.author}</td>
                      <td className="px-6 py-4">
                        <div>
                          <button
                            onClick={() => navigate(`/slide/edit/${slide._id}`)}
              ...
              <nav
                ...
                <span className="block w-full mb-4 text-sm font-normal text-gray-500 dark:text-gray-400 md:mb-0 md:inline md:w-auto">
                  <span className="font-semibold text-gray-900 dark:text-white">
                    {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, slides.length)}
                  </span>{' '}
                  of{' '}
                  <span className="font-semibold text-gray-900 dark:text-white">
                    {slides.length}
